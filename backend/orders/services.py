...
def send_order_success_notifications(user, order):
    ...
    # SMS уведомление
    sms_message_body = f'{recipient_name}, Ваш заказ №{order.slug} успешно оформлен. ' \
    ...
    email_body = f'{recipient_name}, Ваш заказ №{order.slug} успешно оформлен. ' \
                 f'Подробную информацию о заказе можете узнать по ссылке: {link}' 
